...
type AudioContextValue = { track: MusicTrack | null; tracks: MusicTrack[]; isPlaying: boolean; currentTime: number; duration: number; volume: number; selectTrack: (track: MusicTrack, play?: boolean) => void; togglePlay: () => void; seek: (time: number) => void; setVolume: (volume: number) => void };
...
export function AudioProvider({ children }: { children: React.ReactNode }) {
  ...
  return <AudioContext.Provider value={{ track, tracks, isPlaying, currentTime, duration, volume, selectTrack, togglePlay, seek, setVolume }}><audio ref={audioRef} onTimeUpdate={(event) => setCurrentTime(event.currentTarget.currentTime)} onLoadedMetadata={(event) => setDuration(event.currentTarget.duration)} onPlay={() => setIsPlaying(true)} onPause={() => setIsPlaying(false)} onEnded={() => setIsPlaying(false)} /><AudioPlayer />{children}</AudioContext.Provider>;
}

export default function AudioPlayer() {
  const { track, isPlaying, currentTime, duration, volume, togglePlay, seek, setVolume } = useAudioPlayer();
  ...
  return <aside className="audio-player" aria-label="Audio player"><div className="audio-player-info"><span>Now listening</span><strong>{track.title}</strong><small>{track.composer || "Namirembe Cathedral Choir"}</small></div><button className="audio-player-toggle" onClick={togglePlay} aria-label={isPlaying ? "Pause track" : "Play track"}>{isPlaying ? "||" : "▶"}</button><input className="audio-player-progress" type="range" min="0" max={duration || 0} step="0.1" value={Math.min(currentTime, duration || 0)} onChange={(event) => seek(Number(event.target.value))} aria-label="Playback progress" /><label className="audio-player-volume" aria-label="Volume"><span>♪</span><input type="range" min="0" max="1" step="0.05" value={volume} onChange={(event) => setVolume(Number(event.target.value))} /></label><button className="audio-player-close" onClick={() => setHidden(true)} aria-label="Minimize audio player">×</button></aside>;
}
